...
export function BeforeAfter() {
  ...
  return (
    <div ref={(el) => void trackOnView(el, "proof_view")}>
      <p className="text-[18px] text-[var(--color-ink-soft)]">
        {week === 0 ? "До подключения" : `Неделя ${week}`} — потяните шкалу
      </p>

      <div
        ref={frame}
        className="relative bg-[var(--color-leaf-edge)] mt-5 touch-none px-6 py-2 select-none sm:px-10"
      ...
      >
        {BANDS.map((b) => {
          ...
          return (
            ...
            >
              ...
              <span className="block h-5 bg-[var(--color-leaf-edge)]">
                <span
        ...
        <div
          ...
          style={{ left: `${t * 100}%` }}
        >
          <span className="absolute top-1/2 left-1/2 block h-12 w-2.5 -translate-x-1/2 -translate-y-1/2 bg-[var(--color-ink)]" />
        </div>
      ...
      <div className="mt-4 flex justify-between text-[17px] text-[var(--color-ink-soft)]">
        <span>старт</span>
