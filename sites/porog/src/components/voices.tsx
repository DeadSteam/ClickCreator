...
export function Voices() {
  ...
  return (
    <div>
      <Reveal>
        <figure className="border-t border-[var(--color-graphite)] pt-10 lg:grid lg:grid-cols-[1fr_auto] lg:items-start lg:gap-20">
          <blockquote className="max-w-[26ch] text-[clamp(28px,3.4vw,42px)] leading-[1.18] font-bold tracking-[-0.02em]">
            {lead.quote}
          </blockquote>
          <figcaption className="mt-10 flex items-end justify-between gap-10 lg:mt-0 lg:block">
            <div>
              <p className="text-[19px] font-bold">{lead.name}</p>
              <p className="mt-1.5 max-w-[24ch] text-[17px] leading-snug text-[var(--color-graphite-soft)]">
                {lead.role}, {lead.org}
              </p>
            </div>
            <div className="shrink-0 text-right lg:mt-10 lg:text-left">
              <p className="num text-[34px] leading-none font-medium text-[var(--color-graphite)]">
                {lead.figure}
              </p>
              <p className="mt-2 text-[17px] text-[var(--color-graphite-soft)]">
                {lead.note}
              </p>
            </div>
          </figcaption>
        ...
        {rest.map((v, i) => (
          <Reveal key={v.name} delay={0.06 + i * 0.06} className={i === 1 ? "lg:mt-14" : ""}>
            <figure className="h-full border-t border-[var(--color-rule-soft)] pt-8">
              <blockquote className="max-w-[44ch] text-[18px] leading-relaxed text-[var(--color-graphite-soft)]">
                {v.quote}
              </blockquote>
              <figcaption className="mt-6 flex items-baseline justify-between gap-8">
                <div>
                  <p className="text-[18px] font-bold">{v.name}</p>
                  <p className="mt-0.5 text-[17px] text-[var(--color-graphite-soft)]">
                    {v.role}, {v.org}
                  </p>
                </div>
                <p className="num shrink-0 text-[19px] text-[var(--color-graphite)]">
                  {v.figure}
                </p>
              </figcaption>
            </figure>
