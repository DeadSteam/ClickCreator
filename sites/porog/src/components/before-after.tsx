...
export function BeforeAfter() {
  ...
  return (
    <div ref={(el) => void trackOnView(el, "proof_view")}>
      <p className="text-[18px] text-[var(--color-graphite-soft)]">
        {week === 0 ? "До подключения" : `Неделя ${week}`} — потяните шкалу
      </p>

      <div
        ref={frame}
        className="sink relative mt-5 touch-none px-6 py-2 select-none sm:px-10"
      ...
      >
        {BANDS.map((b) => {
          ...
          return (
            ...
            >
              ...
              <span className="block h-5 bg-[var(--color-sheet-sink)]">
                <span
        ...
        <div
          aria-hidden
          className="pointer-events-none absolute inset-y-0 w-px bg-[var(--color-graphite)]"
          style={{ left: `${t * 100}%` }}
        >
          <span className="absolute top-1/2 left-1/2 block h-12 w-2.5 -translate-x-1/2 -translate-y-1/2 bg-[var(--color-graphite)]" />
        </div>
      ...
      <div className="mt-4 flex justify-between text-[17px] text-[var(--color-graphite-soft)]">
        <span>старт</span>
