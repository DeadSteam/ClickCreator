...
export function TrialCta({
  ...
  const link = (
    <a
      ...
      rel="noopener"
      /*
        Метка для `MobileCta` (п.56B.11 мастер-промпта): sticky-панель внизу
        обязана прятаться, пока в кадре уже есть настоящий CTA, — иначе на
        экране одновременно оказываются две конкурирующие кнопки. Наблюдатель
        в `MobileCta` ищет этот атрибут по всей странице, а не только в своём
        поддереве, поэтому связь — через DOM-атрибут, а не через пропс.
      */
      data-cta-marker=""
      /*
